...
export class Store {
  ...
  addAccount(input) {
    return this.mutate((state) => {
      const account = {
        ...
        plan: cleanText(input.plan, 40) || "Plus",
  ...
  addProvider(input) {
    return this.mutate((state) => {
      const type = input.type === "anthropic" ? "anthropic" : "openai";
      const provider = {
        id: id("provider"),
        type,
        label: cleanText(input.label, 80) || (type === "anthropic" ? "Claude API" : "OpenAI API"),
  ...
  addProviderUsage(providerId, usage = {}) {
    return this.mutate((state) => {
      const provider = state.providers.find((item) => item.id === safeId(providerId));
      if (!provider) throw new Error("Không tìm thấy API provider.");
      provider.usage ||= { inputTokens: 0, cachedInputTokens: 0, cacheWriteTokens: 0, outputTokens: 0, totalTokens: 0, calls: 0 };
      for (const field of ["inputTokens", "cachedInputTokens", "cacheWriteTokens", "outputTokens", "totalTokens"]) {
